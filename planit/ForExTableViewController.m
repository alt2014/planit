...
- (void)viewDidLoad
{
    ...
    self.currentRate = [NSDecimalNumber decimalNumberWithString:@"1"];
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row   inComponent:(NSInteger)component
{
    ...
    [_manager.communicator getCurrentCurrencyRates:currency callback:^(NSError *err, Currency *trips) {
        if ([self.fromAmountField.text length] > 0) {
            NSDecimalNumber *factor = [NSDecimalNumber decimalNumberWithString:self.fromAmountField.text];
            NSDecimalNumber *rate = [trips.exchangeRate decimalNumberByMultiplyingBy:factor];
            
            NSString *rateStr = [NSString stringWithFormat: @"%@", rate];
            dispatch_async(dispatch_get_main_queue(), ^{
                self.currentRate = trips.exchangeRate;
...
- (IBAction)fromAmountDidChange:(UITextField *)sender {
    if ([self.fromAmountField.text length] > 0) {
        NSDecimalNumber *factor = [NSDecimalNumber decimalNumberWithString:self.fromAmountField.text];
        NSDecimalNumber *rate = [self.currentRate decimalNumberByMultiplyingBy:factor];
